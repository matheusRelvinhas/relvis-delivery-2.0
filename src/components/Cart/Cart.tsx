...
export default function Header() {
  const { dataCss, isTilted, handleCartClick, cartItems, setCartItems,drinkCards, drinkNoAlcoolCards, portionsCards } = useGlobalContext();
  ...
  return (
    ...
    <div className='cart-message'>
      <strong>{totalItems === 0 ? 'vazio :(' : `${totalItems} itens`}</strong>
    </div>
    </>
  );
}
